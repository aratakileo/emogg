...
public class ModrinthApi {
        ...
        final var request = HttpRequest.newBuilder(URI.create(getRequestUrl()))
                .setHeader(
                        ...
                        "github.com/aratakileo/"
                                ...
                                + '@' + Emogg.getPlatform().getModVersion(Emogg.NAMESPACE_OR_ID)
    ...
    public static @NotNull String getLinkForUpdate() {
        return "https://modrinth.com/mod/emogg/versions?g=" + Emogg.getPlatform().getMinecraftVersion();
    }

    private static boolean isVersionGreaterThanCurrent(@Nullable String version) {
        if (version == null) return false;

        final var currentVerMetadata = getVersionMetadata(Emogg.getPlatform().getModVersion(Emogg.NAMESPACE_OR_ID));
    ...
    /**
     * Supports version format that starts with: `x.x-BETA.x` or `x.x.x`, with `-` or `+` and Minecraft verison name,
     * or without it, where x is digit or number
     */
    ...
    private static @NotNull String getRequestUrl() {
        return REQUEST_URL.replace("{identifier}", Emogg.NAMESPACE_OR_ID)
                .replace("{minecraft_version}", Emogg.getPlatform().getMinecraftVersion());
    }
