...
import pextystudios.emogg.mixin.accessor.FontAccessor;
...
public class EmojiFontRenderer extends Font {
    ...
    public EmojiFontRenderer(Font font) {
        super(((FontAccessor)font).getFonts(), ((FontAccessor)font).isFilterFishyGlyphs());
    }
    ...
    public int drawInBatch(FormattedCharSequence formattedCharSequence, float x, float y, int color, boolean shadow, Matrix4f matrix4f, MultiBufferSource multiBufferSource, DisplayMode displayMode, int backgroundColor, int light) {
        ...
        if (shadow) {
            ...
            matrix4f.translate(FontAccessor.getSHADOW_OFFSET());
        }
    ...
    class EmojiCharSink implements FormattedCharSink {
        ...
        public void finish(int color, float x_off) {
            ...
            if (!effects.isEmpty()) {
                FontSet fontSet = ((FontAccessor)EmojiFontRenderer.this).callGetFontSet(Style.DEFAULT_FONT);
        ...
        public boolean accept(int index, Style style, int codePoint) {
            ...
            final var fontSet = ((FontAccessor)EmojiFontRenderer.this).callGetFontSet(style.getFont());
            final var glyph = fontSet.getGlyphInfo(codePoint, ((FontAccessor)EmojiFontRenderer.this).isFilterFishyGlyphs());
            ...
            if (!(bakedGlyph instanceof EmptyGlyph)) {
                ...
                ((FontAccessor)EmojiFontRenderer.this).callRenderChar(
                        bakedGlyph,
                        style.isBold(),
                        style.isItalic(),
