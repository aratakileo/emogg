...
public class EmojiSelectionMenu extends AbstractWidget {
    ...
    protected EmojiSelectionMenu(float emojiSize, int headerHeight) {
        ...
        this.scrollbarThumbRect = new RenderUtil.Rect2i(
                scrollbarRect.getX() + 1,
                scrollbarRect.getY() + 1,
                SCROLLBAR_WIDTH - 2,
                Math.max(8, scrollbarRect.getHeight() - 2 - (totalLinesAmount - MAX_NUMBER_OF_LINES_ON_PAGE) / 2)
        );
    ...
    public void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float dt) {
        ...
        final var renderableSettingsButtonRect = settingsButtonRect.move(getX(), getY());

        /*
        * Start of header processing & rendering
        */
        RenderUtil.drawRect(getX(), getY(), width, (int) emojiSize, 0xaa000000);
        ...
        for (var segmentStartIndex: segments.keySet()) {
            ...
            for (var emoji: currentSegment.getB()) {
                if (iline >= 0) {
                    ...
                    final var emojiY = (int) (getY() + emojiSize + iline * (emojiSize + 1) + 1);
        ...
        */
        ...
        RenderUtil.drawRect(
                scrollbarRect.move(getX(), getY()),
                0xaa222222,
                1,
                0xaa000000
        );

        final var renderableScrollbarThumbRect = scrollbarThumbRect.move(getX(), getY());
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double scrollDelta) {
        ...
        scrollbarThumbRect.setY(
                (int) (scrollbarRect.getY() + 1 + (scrollbarRect.getHeight() - 2 - scrollbarThumbRect.getHeight()) * (
    ...
    public boolean mouseDragged(double mouseX, double mouseY, int button, double deltaX, double deltaY) {
        ...
        applyScrollByThumbY((int)(mouseY - getY() - scrollingThumbMouseOffset));
    ...
    public void onClick(double mouseX, double mouseY) {
        if (settingsButtonRect.move(getX(), getY()).contains((int) mouseX, (int) mouseY)) {
            playClickSound();
        ...
        if (renderableScrollbarThumbRect.contains((int)mouseX, (int)mouseY)) {
            scrollingThumbMouseOffset = (int) (mouseY - renderableScrollbarThumbRect.getY());
            return;
        }

        if (!scrollbarRect.move(getX(), getY()).contains((int)mouseX, (int)mouseY)) return;

        applyScrollByThumbY((int) (mouseY - getY()));
    }
    ...
    private void applyScrollByThumbY(int y) {
        final var scrollbarFieldHeight =  scrollbarRect.getHeight() - 2 - scrollbarThumbRect.getHeight();

        scrollbarThumbRect.setY(Math.min(Math.max(y, headerHeight + 1), scrollbarFieldHeight + headerHeight + 1));
        scrollLinesAmount = (int) ((
                totalLinesAmount - MAX_NUMBER_OF_LINES_ON_PAGE
        ) * (
                (double)(scrollbarThumbRect.getY() - headerHeight - 1) / (double)(scrollbarFieldHeight))
        );
    }
